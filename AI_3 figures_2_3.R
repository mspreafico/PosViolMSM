...
for(nn in 1:length(n_sizes)){
  ...
  for(pp in 1:length(pis)){
    ...
    nowt[[nn]][[pp]] = plot.survI(data = surv_perfs$eval[n==n_sizes[nn] & pi==pis[pp] & WT=='NoWT'],
...
plot.n = ggarrange(nowt[[nn1]][[2]]+rremove("ylab"), 
                  nowt[[nn1]][[3]]+rremove("ylab"), 
                  nowt[[nn1]][[4]]+rremove("ylab"), 
                  nowt[[nn1]][[5]]+rremove("ylab"),
                  nowt[[nn1]][[6]]+rremove("ylab"), 
                  nowt[[nn1]][[7]]+rremove("ylab"),
                  nowt[[nn2]][[2]]+rremove("ylab"),
                  nowt[[nn2]][[3]]+rremove("ylab"), 
                  nowt[[nn2]][[4]]+rremove("ylab"),
                  nowt[[nn2]][[5]]+rremove("ylab"),
                  nowt[[nn2]][[6]]+rremove("ylab"),
                  nowt[[nn2]][[7]]+rremove("ylab"), 
...
for(nn in 1:length(n_sizes)){
  ...
  for(pp in 1:length(pis)){
    current.title = substitute(paste('No WT, ',pi == number2),
                               list(number1 = n_sizes[nn], number2 = pis[pp]))
    nowt2[[nn]][[pp]] = plot.survI(data = surv_perfs$eval[n==n_sizes[nn] & pi==pis[pp] & WT=='NoWT'],
                                  title = current.title, title.rel = 0.95)
  }
}
...
for(nn in 1:length(n_sizes)){
  ...
  for(pp in 1:length(pis)){
    current.title = substitute(paste('WT: 1-99, ',pi == number2),
                               list(number1 = n_sizes[nn], number2 = pis[pp]))
    wt1[[nn]][[pp]] = plot.survI(data = surv_perfs$eval[n==n_sizes[nn] & pi==pis[pp] & WT=='1-99'],
                                 title = current.title, title.rel = 0.95)
  }
}
...
for(nn in 1:length(n_sizes)){
  ...
  for(pp in 1:length(pis)){
    current.title = substitute(paste('WT: 5-95, ',pi == number2),
                               list(number1 = n_sizes[nn], number2 = pis[pp]))
    wt5[[nn]][[pp]] = plot.survI(data = surv_perfs$eval[n==n_sizes[nn] & pi==pis[pp] & WT=='5-95'],
                                 title = current.title,  title.rel = 0.95)
  }
}
...
for(nn in 1:length(n_sizes)){
  ...
  for(pp in 1:length(pis)){
    current.title = substitute(paste('WT: 10-90, ',pi == number2),
                               list(number1 = n_sizes[nn], number2 = pis[pp]))
    wt10[[nn]][[pp]] = plot.survI(data = surv_perfs$eval[n==n_sizes[nn] & pi==pis[pp] & WT=='10-90'],
                                  title = current.title,  title.rel = 0.95)
  }
}
